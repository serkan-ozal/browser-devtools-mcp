...
export interface GoBackInput extends ToolInput {
    timeout?: number;
    waitUntil?: 'load' | 'domcontentloaded' | 'networkidle' | 'commit';
}

export interface GoBackOutput extends ToolOutput {
...
export class GoBack implements Tool {
    name(): string {
...
- \`commit\`: Consider operation to be finished when network response is received and the document started loading.`
    ...
    async handle(
        ...
        args: GoBackInput
    ): Promise<GoBackOutput> {
        const response: Response | null = await context.page.goBack({
