...
export interface GoForwardInput extends ToolInput {
    timeout?: number;
    waitUntil?: 'load' | 'domcontentloaded' | 'networkidle' | 'commit';
}

export interface GoForwardOutput extends ToolOutput {
...
export class GoForward implements Tool {
    name(): string {
...
- \`commit\`: Consider operation to be finished when network response is received and the document started loading.`
    ...
    async handle(
        ...
        args: GoForwardInput
    ): Promise<GoForwardOutput> {
        const response: Response | null = await context.page.goForward({
